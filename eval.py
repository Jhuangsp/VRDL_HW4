...
from TA_utils import binary_mask_to_rle
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str='', VRDL=False):
    """
    ...
    if num_dets_to_consider == 0:
        if VRDL:
            return img_numpy, np.array([]), np.array([]), np.array([])
        else:
            return img_numpy
    ...
    if VRDL:
        return img_numpy, classes[:num_dets_to_consider], masks[:num_dets_to_consider], scores[:num_dets_to_consider]
    else:
        return img_numpy    
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    img_numpy, VRDL_classes, VRDL_masks, VRDL_scores = prep_display(preds, frame, None, None, undo_transform=False, VRDL=True)
    print('Class')
    pp.pprint(VRDL_classes)
    print('Masks')
    print(VRDL_masks.shape)
    print('Scores')
    pp.pprint(VRDL_scores) 
    ...
    return VRDL_classes, VRDL_masks, VRDL_scores
...
def evalimages(net:Yolact, input_folder:str, output_folder:str):
    ...
    print()
    coco = COCO("./data/VRDL_data/annotations/test.json")
    # coco = COCO("./data/VRDL_data/annotations/pascal_train.json")
    ...
    # for p in Path(input_folder).glob('*'): 
        # path = str(p)
    for k in keys:
        path = './data/VRDL_data/images/test_images/' + coco.imgs[k]['file_name']
        # path = './data/VRDL_data/images/train_images/' + coco.imgs[k]['file_name']
        name = os.path.basename(path)
        name = '.'.join(name.split('.')[:-1]) + '.png'
        out_path = os.path.join(output_folder, name)

        VRDL_classes, VRDL_masks, VRDL_scores = evalimage(net, path, out_path) 
        for i in range(len(VRDL_scores)):
            pred = {}
            pred['image_id'] = k
            pred['category_id'] = int(VRDL_classes[i]) + 1
            pred['segmentation'] = binary_mask_to_rle(VRDL_masks[i,:,:,0].cpu().numpy())
            pred['score'] = float(VRDL_scores[i])
            coco_dt.append(pred)
        print(path + ' -> ' + out_path)
