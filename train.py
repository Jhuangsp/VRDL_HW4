...
.decay)
    ...
    save_path = lambda epoch, iteration, best=False: SavePath(cfg.name + '_best_' if best else '', epoch, iteration).get_path(root=args.save_folder)
        ...
        for epoch in range(num_epochs):
            ...
            if args.validation_epoch > 0:
                if epoch % args.validation_epoch == 0 and epoch > 0:
                    mask_map = compute_validation_map(epoch, iteration, yolact_net, val_dataset, log if args.log else None)
                    if mask_map > best_map:
                        print('Best map raise from {} to {}'.format(best_map, mask_map))
                        yolact_net.save_weights(save_path(epoch, iteration, True))
                        best_map = mask_map
                    else:
                        print('map: {} (Best map {})'.format(mask_map, best_map))
